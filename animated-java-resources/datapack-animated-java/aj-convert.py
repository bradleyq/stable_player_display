...
if nArgs < 2:
    ...
    print("\t-ns=[namespace]\t\tinternal project namespace. Default 'zzzzzzzz'")
...
print("running aj-convert on project " + project + " with internal namespace " + namespace)

# construct summon.mcfunction path
summonPath =  os.path.join(".", "data", "animated_java", "functions", project, "summon.mcfunction")
summonResult = ""
...
defaultBonePath =  os.path.join(".", "data", "animated_java", "functions", project, namespace, "apply_variant", "default", "as_bone.mcfunction")
defaultBoneResult = ""

# read default/as_bone.mcfunction
f = open(defaultBonePath, "r")

for line in f:
    for offsetPair in offsets:
        if "[tag=aj." + project + ".bone." + offsetPair[0] + "]" in line:
            line = line.strip()[:-1] + str(offsetPair[2]) + "\n"
    defaultBoneResult += line

f.close()

# write to default/as_bone.mcfunction
f = open(defaultBonePath, "w")
f.write(defaultBoneResult)
f.close()

# construct tree path
animationPath = os.path.join(".", "data", "animated_java", "functions", project, namespace, "animations")
